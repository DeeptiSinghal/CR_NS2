...
\"\n\
checkout_executable TCLSH \"/home/deepti/NS/ns-allinone-2.34/bin/tclsh8.4\" tclsh \"\\\n\
When configured, ns found the right version of tclsh in\\\n\
/home/deepti/NS/ns-allinone-2.34/bin/tclsh8.4\n\
but it doesn't seem to be there anymore, so\\\n\
...
$ns mac-type $mactype\n\
\n\
set channelnum [$ns get-numCHs]  \n\
if {$channelnum!=\"\"} { \n\
for { set j 0} {$j < $channelnum} {incr j} {\n\
set channel_($j) [$ns get-channel $j]\n\
}\n\
} else {\n\
set channel [$ns get-channel 0]\n\
}\n\
\n\
set inerr_($t) \"\"\n\
if {$inerrproc != \"\"} {\n\
...
set fec $fec_($t)\n\
\n\
set arptable_($t) [new ARPTable $self $mac_($t)]\n\
set arptable $arptable_($t)\n\
\n\
if {$channelnum!=\"\"} {\n\
$arptable numChannel $channelnum\n\
}	\n\
\n\
if {$imepflag != \"\"} {\n\
...
\n\
if {$channelnum!=\"\"} { 				;# for every single-interface, add multi-channel\n\
for { set i 0} {$i<$channelnum} {incr i} {\n\
$netif mchannel $channel_($i)\n\
}\n\
} else {\n\
...
if {$channelnum!=\"\"} { 				;# for every single-interface, add multi-channel_\n\
$topo numChannel $channelnum\n\
for { set i 0} {$i<$channelnum} {incr i} {\n\
$channel_($i) addif $netif \n\
$channel_($i) add-node $self \n\
$topo mchannel $i $channel_($i)\n\
}\n\
} else {\n\
$channel addif $netif\n\
